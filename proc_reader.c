...
int list_process_directories(void) {
    ...
    if (closedir(dir) == -1) {
        perror("ERROR: Failed to close /proc directory");
        return -1;
    }
...
int show_system_info(void) {
    ...
    if (fclose(file) == EOF) {
        perror("ERROR: Failed to close /proc/cpuinfo");
        return -1;
    }
    ...
    if (file == NULL) {
        perror("ERROR: Failed to open /proc/meminfo");
        return -1;
    }
    ...
    if (fclose(file) == EOF) {
        perror("ERROR: Failed to close /proc/meminfo");
        return -1;
    }
